...
def betterEvaluationFunction(currentGameState):
    ...
    for (xg, yg), t in zip(ghostPositions, scaredTimes):
        if t>0:
            scaredGhostScore += -50 * (abs(xp - xg) + abs(yp - yg))
        else:
            dist = abs(xp - xg) + abs(yp - yg)
            if dist < 1:
                normalGhostScore += -100000./(dist + .1)
                farGhosts = False
            #else:
    ...
    closestFood = 10000, 10000
    minDist = 200
    for x in range(foodMatrix.width):
        for y in range(foodMatrix.height):
            if foodMatrix[x][y] == True:
                if(abs(x - xp) + abs(y - yp) < minDist):
                    closestFood = x, y
                    minDist = abs(x - xp) + abs(y - yp)
                #val += 10000 - (abs(x - xp) + abs(y - yp) + .01)**2
