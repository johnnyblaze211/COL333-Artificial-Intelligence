...
if __name__ == '__main__':
    params1 = pd.read_csv('input_a.csv')
    np_arr = []
    bool_arr = []
    for i in range(params1.shape[0]):
        #print(params1.iloc[0])
        #sys.exit()
        ...
        if(r + a < m):
            ...
            b = check_constraints_a(N, D, m, a, e, r, arr)
            if not b: no_solution = True
    ...
    with open("solution.json", 'w') as file:
        for i in range(len(bool_arr)):
            if bool_arr[i]:
                json.dump(numpy_to_dict(np_arr[i]), file)
                file.write("\n")
            else:
                json.dump({}, file)
                file.write("\n")
